...
#                                            # 
# https://github.com/Polsulpicien/ip-scanner # 
############################################## 
import socket
import os
    ...
    def lookup(self, address):
        ...
        try:
            ...
            with open('ip.txt', 'a') as f:
                f.write(f">> {str(address)} => {str(hostname)}\n")
            self.send(address, hostname)
        except socket.herror:
            if show=="true" or show=="yes":
                with open('ip.txt', 'a') as f:
                    f.write(f"- {str(address)} => None\n")
...
class Netscan():
    ...
    def addresses(self):
        ...
        #try 16-bit	                        #
        #########################################
        if self.mode=="16" or self.mode=="all" or self.mode=="tens":
            for ping in range(0, 256):
                for ping_ in range(0, 256):
                    addresses.append(f"192.168.{ping}.{ping_}")
        #########################################
        #try 20-bit	                        #
        #########################################
        if self.mode=="20" or self.mode=="all" or self.mode=="twenties" or self.mode=="all":
            for ping in range(16, 32):
                for ping_ in range(0, 256):
                    for ping__ in range(0, 256):
                        addresses.append(f"172.{ping}.{ping_}.{ping__}")
        #########################################
        #try 24-bit	                        #
        #########################################
        if self.mode=="24" or self.mode=="all" or self.mode=="twenties":
            for ping in range(0, 256):
                for ping_ in range(0, 256):
                    for ping__ in range(0, 256):
                        addresses.append(f"172.{ping}.{ping_}.{ping__}")
        ...
        with open('ip.txt', 'w') as f:
            f.write(f'''###########################################################################################
#                                                                                         #
#  IP SCANNER • github.com/Polsulpicien • discord.gg/xm9QX3Q • Made by Polsulpicien#5020  #
#                                                                                         #
###########################################################################################
IP LIST:
''')
...
class Menu():
    ...
    def title(self):
        ...
        return print(f'''
            ...
            {fg(127)}██║██╔═══╝    ╚═══██╗██║  ██╗██╔══██║██║╚████║██║╚████║██╔══╝  ██╔══██╗
            {fg(128)}██║██║       ██████╔╝╚█████╔╝██║  ██║██║ ╚███║██║ ╚███║███████╗██║  ██║
            {fg(129)}╚═╝╚═╝       ╚═════╝  ╚════╝ ╚═╝  ╚═╝╚═╝  ╚══╝╚═╝  ╚══╝╚══════╝╚═╝  ╚═╝ github.com/Polsulpicien
